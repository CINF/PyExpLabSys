    ...
    def run(self):
        while not quit:
            try:
                ...
                self.chiller_flow = network_comm('130.225.86.120', 9759, 'read_flow_rate')
                self.chiller_temp_amb = network_comm('130.225.86.120', 9759, 'read_ambient_temperature')
                self.chiller_pressure = network_comm('130.225.86.120', 9759, 'read_pressure')
                self.chiller_temp_setpoint = network_comm('130.225.86.120', 9759, 'read_setpoint')
                time.sleep(5)
            except:
    ...
    def run(self):
        while not quit:
            ...
            if (time_trigged):
                ...
                meas_time = sqlTime()
                sql_pressure = "insert into chiller_sputterchamber_pressure set time=\"" +  meas_time + "\", pressure = " + str(self.reader.chiller_pressure)
                sql_flow = "insert into chiller_sputterchamber_flow set time=\"" +  meas_time + "\", flow = " + str(self.reader.chiller_flow)
                sql_temperature = "insert into chiller_sputterchamber_temperature set time=\"" +  meas_time + "\", temperature = " + str(self.reader.chiller_temp)
                sql_temperature_amb = "insert into chiller_sputterchamber_temperature_ambient set time=\"" +  meas_time + "\", temperature = " + str(self.reader.chiller_temp_amb)
                sql_temperature_setpoint = "insert into chiller_sputterchamber_temperature_setpoint set time=\"" +  meas_time + "\", temperature = " + str(self.reader.chiller_temp_setpoint)
