...
from PyExpLabSys.drivers.deltaco_TB_298 import detect_keypad_device, ThreadedKeypad, KEYS_TO_CHARS, STOP
...
class TUI(object):
    ...
    def absolute_mode(self):
        ...
        while True:
            ...
            # Check wether numpad thread is alive
            if time.time() - t0 > 60:
                print('{}\nNumpad thread: {}'.format(time.asctime(), numpad.is_alive()))
                t0 = time.time()

            # Listen for key press
            try:
                key = numpad.key_pressed_queue.get(timeout=0.01)
                #print('Absolute mode - key press: {}'.format(key))
            except Empty:
                continue
            ...
            if key == STOP:
                t_error = time.time()
                self.draw_box(self.box4, action=['clear'])
                new_x, new_y = self.box4['origin']
                self.picaso.move_origin(new_x, new_y)
                self.picaso.put_string('STOP encountered - see terminal')
                input('Press ENTER to continue and see time since STOP encountered')
                t_error = time.time() - t_error
                hours = int(t_error/3600)
                minutes = int((t_error-3600*hours)/60)
                seconds = t_error - 3600*hours - 60*minutes
                print('STOP encountered:')
                print('{} hours {} minutes and {} seconds ago'.format(hours, minutes, seconds))
                
            # Continue if NUMLOCK is deactivated
            if not numpad.led_on:
                continue
