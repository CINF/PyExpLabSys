...
setpoint = 8
max_rate = 34.38
rate_second_start = 1.5
rate_main_start = 2
rate_second_max = 1.7
# Edit starting parameters

## Edit comment
comment = "pH 8 - p: 0.02. i: 0. start rate: 1.5."
## Edit comment
...
def run(pump_second, pump_main, obus, pid, data_set_saver, time_start, setpoint, rate_second, rate_main, rate_second_offset, rate_main_offset, rate_second_min, rate_second_max, direction):
...
#        n = n + 1
        ...
        if value > setpoint-0.5 or value == setpoint-0.5:
            ...
            #Ensure that secondary rate stays within specified interval
        ...
        if value < setpoint+0.5 or value == setpoint+0.5:
            if rate_second == 0:
                pump_second.set_rate(round(rate_secondary_offset,3))
...
def main():
    ...
    parser.add_argument('--direction', default=None, help='..', required=True)
    parser.add_argument('--main-offset', type=float, default=None, help='..', required=True)
    parser.add_argument('--second-min', type=float, default=None, help='..', required=True)
    parser.add_argument('--second-max', type=float, default=None, help='..', required=True)
    parser.add_argument('--second-offset', type=float, default=None, help='..', required=True)
...
#    raise SystemExit('All good')
    ...
    pid = PID(pid_p=0.02, pid_i=0.0, pid_d=0, p_max=9999, p_min=0.001)
    ...
    if args.direction == "up":
        pump_second.set_rate(args.secondary_offset)
        rate_second = args.secondary_offset
    ...
    # Run PID
    try:
        run(pump_second, pump_main, obus, pid, data_set_saver, time_start, args.setpoint, rate_second, rate_main, args.second_offset, args.main_offset, args.second_min , args.second_max, args.direction)
    except:
        # Clean up
        pump_main.set_rate(0)
        pump_second.set_rate(0)
