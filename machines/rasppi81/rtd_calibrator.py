...
class rtd_calibrator(object):
    ...
    def start_measurement(self, comment):
        ...
        metadata = {"Time": CustomColumn(data_set_time, "FROM_UNIXTIME(%s)"), "comment": comment,
                    "type": 65, "preamp_range": 0, "sem_voltage": 0}

        labels = ["2W Heater 1", "2W Heater 2", "2W RTD Source", "2W RTD Sense", "Short circuit check",
                      "RTD Source Sense Left", "RTD Source Sense Right", "4W RTD", "Temperature"]
        ...
        for i in range(0, 2):
            ...
            for label in labels:
                ...
                data_set_saver.save_point(label, (time.time() - data_set_time, self.data[label]))
            time.sleep(1)


    
if __name__ == '__main__':
    CALIBRATOR = rtd_calibrator('USB0::0x0957::0x2007::MY57000209::INSTR')
