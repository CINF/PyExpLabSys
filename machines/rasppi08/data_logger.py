...
class Reader(threading.Thread):
    def __init__(self, iongauge, omega, codenames, pullsocket, livesocket):
    ...
    def run(self):
        ...
        while not self.quit:
            ...
            self.livesocket.set_point_now(self.codenames[0], pressure)
            ...
            self.livesocket.set_point_now(self.codenames[1], temperature)
            time.sleep(0.2)
        else:
...
if __name__ == '__main__':
    ...
    livesocket = LiveSocket('nanobeam_live', codenames)
    livesocket.start()

    reader = Reader(iongauge, omega, codenames, pullsocket, livesocket)
    reader.daemon = True
    reader.start()
    ...
    while run:
        try:
            ...
            if reader.isAlive():
                ...
                raise KeyboardInterrupt('Reader has stopped')
        except:
