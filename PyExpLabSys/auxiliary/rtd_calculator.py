...
class RtdCalculator(object):
    ...
    def __init__(self, calib_temperature, calib_resistance, material='Pt'):
        ...
        self.resistance_at_0 = calib_resistance / (
            1 + self.coeff_a * calib_temperature + self.coeff_b * calib_temperature ** 2
        )

    def find_r(self, temperature):
        """ Find the resistance for a given temperature """
        resistance = self.resistance_at_0 * (
            1 + self.coeff_a * temperature + self.coeff_b * temperature ** 2
        )
        return resistance
