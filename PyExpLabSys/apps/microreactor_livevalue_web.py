        ...
        html.Div(
            [
                html.Div(
                    [
                        html.Div(
                            [dcc.Graph(id="plot_press", animate=True, className="twelve")],
                            className="row",
                        ),
                        ### Three Plots ###
                        html.Div(
...
def update_press_graph(n):
    ...
    data = [
        ...
        go.Scatter(ALL_DATA["buffer_pressure"], marker=dict(color=COLOURS["buffer_pressure"])),
    ]
    layout = go.Layout(
        xaxis=dict(
            range=[
...
def update_temp_graph(n):
    ...
    if (
        len(
            [
                i
                for i in (ALL_DATA["thermocouple_temp"]["y"] + ALL_DATA["rtd_temp"]["y"])
                if i is not None
            ]
        )
        == 0
