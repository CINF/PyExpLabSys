...
def all_values_update():
    ...
    chamber_pressure = communicate_sock(
        'microreactorng', 'read_pressure#labview', port=7654
    )
    ...
    containment_pressure = communicate_sock(
        'microreactorng', 'read_containment#labview', port=7654
    )
...
APP.layout = html.Div(
    style={'backgroundColor': COLOURS['paper_bgcolor']},
    children=[
        dcc.Interval(
        ...
        html.Div(
            [
                html.Div(
                    [
                        html.Div(
                            [
                                dcc.Graph(
                                    id='plot_press', animate=True, className='twelve'
                                )
                            ],
                            className='row',
                        ),
                ...
                ### Table ###
                html.Div([html.Table(id='table_pressure')], className='four columns'),
            ],
            className='row',
        ),
...
def update_press_graph(n):
    ...
    lst = (
        ALL_DATA['containment_pressure']['y']
    ...
    data = [
        ...
        go.Scatter(
            ALL_DATA['reactor_pressure'], marker=dict(color=COLOURS['reactor_pressure'])
        ),
        go.Scatter(
            ALL_DATA['buffer_pressure'], marker=dict(color=COLOURS['buffer_pressure'])
        ),
    ]
    layout = go.Layout(
        xaxis=dict(
            range=[
                ALL_DATA['thermocouple_temp']['x'][0],
    ...
    if (
        len(
            [
                i
                for i in (
                    ALL_DATA['thermocouple_temp']['y'] + ALL_DATA['rtd_temp']['y']
                )
                if i is not None
            ]
        )
        ...
        y_axis = [
            i
            for i in (ALL_DATA['rtd_temp']['y'] + ALL_DATA['thermocouple_temp']['y'])
            if i is not None
        ]
        ymin = min(y_axis) * 0.999
        ymax = max(y_axis) * 1.001
    data = [
        go.Scatter(ALL_DATA['rtd_temp'], marker=dict(color=COLOURS['rtd_temp'])),
        go.Scatter(
...
def update_flow_graph(n):
    ...
    if len([i for i in lst if i is not None]) == 0:
        ...
        if ymax < 5.5:
            ...
            ymax = 10
    data = [
        go.Scatter(ALL_DATA['flow1'], marker=dict(color=COLOURS['flow1'])),
        go.Scatter(ALL_DATA['flow2'], marker=dict(color=COLOURS['flow2'])),
        go.Scatter(ALL_DATA['flow3'], marker=dict(color=COLOURS['flow3'])),
        go.Scatter(ALL_DATA['flow4'], marker=dict(color=COLOURS['flow4'])),
        go.Scatter(ALL_DATA['flow5'], marker=dict(color=COLOURS['flow5'])),
        go.Scatter(
            ALL_DATA['flow6'],
    ...
    out = (
        ...
        + [
            html.Tr(
                [
                    html.Td(
                        style={'backgroundColor': COLOURS['main_chamber_pressure']}
                    ),
