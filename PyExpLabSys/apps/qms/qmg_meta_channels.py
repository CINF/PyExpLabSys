...
class UdpChannel(threading.Thread):
    ...
    def __init__(self, channel_data, qms):
        """ Initalize the instance of the class """
        threading.Thread.__init__(self)
        self.channel_data = channel_data.copy()
        LOGGER.error(channel_data)
        self.qms = qms
        self.daemon = True

    def run(self):
        start_time = time.time() # WATCH OUT, THIS IS NOT SYNCHRONIZED!!!!!!!!!!!!
        while True:
