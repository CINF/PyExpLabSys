...
class QmsStatusOutput(threading.Thread):
    ...
    def __init__(
        self, qms_instance, sql_saver_instance=None, meta_channel_instance=None
    ):
    ...
    def run(self):
        while True:
            ...
            if self.qms.operating_mode == "Mass Time":
                try:
                    timestamp = "Timestamp: " + self.qms.current_timestamp.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                except AttributeError:
                    timestamp = 'Timestamp: None'
                self.screen.addstr(3, 1, timestamp)
                runtime = "Experiment runtime: {0:.1f}s  ".format(
                    self.qms.measurement_runtime
                )
                ...
                self.screen.clrtoeol()
                self.screen.addstr(
                    6, 1, 'Emission: {}'.format(self.qms.qmg.state['emission'])
                )
                self.screen.clrtoeol()
                self.screen.addstr(
                    6, 40, 'SEM status: {}'.format(self.qms.qmg.state['sem'])
                )
                self.screen.clrtoeol()
            ...
            if not self.sql is None:
                ...
                commit_time = "Last commit duration: {0:.1f}".format(
                    self.sql.commit_time
                )
