...
class QMS(object):
    ...
    def create_mysql_measurement(
        ...
        query = 'select id from measurements_{} order by id desc limit 1'.format(
            self.chamber
        )
        LOGGER.info(query)
        cursor.execute(query)
    ...
    def meta_channels_only(self, timestamp, no_save=False):
        ...
        start_time = (
            time.mktime(timestamp.timetuple()) + timestamp.microsecond / 1000000.0
        )
    ...
    def mass_time(self, ms_channel_list, timestamp, no_save=False):
        ...
        while self.stop is False:
            if (
                time.time() - last_mass_scan_time
                > ms_channel_list[0]['mass-scan-interval']
            ):
            ...
            # time.sleep(0.01)
            save_values = (
                True  # Will be set to false if we do not trust values for this scan
            )
            for channel in range(1, number_of_channels + 1):
                ...
                if value == "":
                    ...
                    query = (
                        'insert into xy_values_{} set measurement = "{}", '
                        + 'x="{}", y="{}"'
                    ).format(
                        self.chamber,
                        ids[channel],
