...
def fit_peak(flight_times, data, ax=None):
    ...
    if ax is not None:
        ...
        if len(flight_times) == 1:
            ...
            ax.plot(X_values, gaussian(X_values, result.params['amp'].value,
                                       result.params['cen'].value,
                                       result.params['wid'].value) + background, 'r-')
        if len(flight_times) == 2:
            #ax.plot(X_values, result.init_fit, 'k--')
            ax.plot(X_values, double_gaussian(X_values, result.params['amp'].value,
                                              result.params['cen'].value,
...
db = mysql.connector.connect(host="localhost", port=9999, user="cinf_reader",passwd = "cinf_reader", db = "cinfdata")
cursor = db.cursor()

#spectrum_numbers = range(4160, 4255)
spectrum_numbers = range(4160, 4200)
    ...
    for x in x_values:
        ...
        for i in range(0, len(x_values[x]['names'])):
            ...
            if usefull:
                ...
                x_values[x]['errors'][name].append(math.sqrt(area * 2500) / spectrum_info[2]) 
            else:
...
for x in x_values:
    for i in range(0, len(x_values['names'])):
        try:
            axis.errorbar(timestamps, x_values[x]['peak_area'], linestyle='-',
                          marker='o', label=x_values[x]['names'][i],
                          yerr=x_values[x]['errors'])
