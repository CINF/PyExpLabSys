...
class Motor(object):
    ...
    def __init__(self, port, slave_adress=1):
        self.instrument = minimalmodbus.Instrument(port, slave_adress, mode = minimalmodbus.MODE_RTU)
    ...
    def get_initial_position(self, operation_number):
        response = self.instrument.read_long(1024+operation_number*2, signed=True)/100
        return response
    ...
    def set_initial_position(self, operation_number, setting):
        ...
        self.instrument.write_long(1024+operation_number*2, setting*100, signed=True)
    ...
    def set_ISS_location(self, setting):
        operation_number = 2
        self.instrument.write_long(1024+operation_number*2, int(setting*100), signed=True)
