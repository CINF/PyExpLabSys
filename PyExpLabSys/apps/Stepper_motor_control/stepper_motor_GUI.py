...
class stepper_motor_GUI(object):
    def __init__(self):
        ...
        self.constant_update.data_update.connect(self.update, )
    ...
    def update(self, data):
        ...
        new_message = '[' + str(datetime.fromtimestamp(data['message'][0]))[11:19] + ']: ' + str(data['message'][1])
    ...
    def send_command(self, command):
        ...
        message = 'raw_wn#command:str:' + '[' + str(datetime.fromtimestamp(time.time()))[11:19].replace(':','-') + '] ' + command
    ...
    def table_update(self):
        ...
        parameterlist = ['operating_speed', 'starting_speed', 'starting_changing_rate', 
                      ...
                      'zhome_operating_speed', 'zhome_starting_speed', 'zhome_acceleration_deceleration', 'group_id']
        for i in range(4):
            for n in range(13):
    ...
    def table_save(self):
        parameterdic = {'command': '[' + str(datetime.fromtimestamp(time.time()))[11:19].replace(':','-') + '] ' + 'table_save', 'operating_speed': [0,0,0,0], 'starting_speed': [0,0,0,0], 'starting_changing_rate': [0,0,0,0], 
                      'stopping_deceleration': [0,0,0,0], 'operating_current': [0,0,0,0], 'positive_software_limit': [0,0,0,0], 
                      'negative_software_limit': [0,0,0,0], 'electronic_gear_A': [0,0,0,0], 'electronic_gear_B': [0,0,0,0], 
                      'zhome_operating_speed': [0,0,0,0], 'zhome_starting_speed': [0,0,0,0], 'zhome_acceleration_deceleration': [0,0,0,0], 
                      'group_id': [0,0,0,0], 'ISS': [0,0,0], 'Mg_XPS': [0,0,0], 'Al_XPS': [0,0,0], 'SIG': [0,0,0], 'HPC': [0,0,0], 'Baking': [0,0,0]}
