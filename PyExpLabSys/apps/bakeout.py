...
class CursesTui(threading.Thread):
    ...
    def run(self):
        while not self.baker.quit:
            ...
            self.screen.addstr(
                5, 2, tui_string.format(time.time() - self.watchdog.timer) + '  '
            )
            self.screen.addstr(
                6, 2, "Watchdog safe: " + str(self.watchdog.watchdog_safe) + ' '
            )
            ...
            for i in range(1, 7):
                self.screen.addstr(
                    13, 7 * i, str(self.baker.dutycycles[i - 1]) + '    '
                )
...
class Bakeout(threading.Thread):
    ...
    def __init__(self):
        ...
        self.pullsocket = DateDataPullSocket(
            self.setup + '-bakeout', channels, timeouts=None
        )
