    ...
    def run(self):
        while True:
            ...
            self.screen.addstr(
                4,
                2,
                "Calculated voltage: {0:.2f}V      ".format(self.eci.wanted_voltage),
            )
            self.screen.addstr(
                5,
                2,
                "Filament voltage: {0:.2f}V     ".format(self.eci.filament["voltage"]),
            )
            self.screen.addstr(
                6,
                2,
                "Filament current: {0:.2f}A    ".format(self.eci.filament["current"]),
            )
            if self.eci.filament["current"] > 0.01:
                self.screen.addstr(
                    5,
                    40,
                    "Filament resisance: {0:.2f}Ohm      ".format(
                        self.eci.filament["voltage"] / self.eci.filament["current"]
                    ),
                )
            else:
            ...
            self.screen.addstr(
                8,
                2,
                "Grid Voltage: {0:.2f}V       ".format(self.eci.bias["grid_voltage"]),
            )
            self.screen.addstr(
                8,
                40,
                "Grid Current: {0:.3f}A       ".format(self.eci.bias["grid_current"]),
            )
            self.screen.addstr(
                12,
                2,
                "Emission current: {0:.4f}mA    ".format(self.eci.emission_current),
            )
            self.screen.addstr(12, 40, "Setpoint: {0:.2f}mA".format(self.eci.setpoint))
            self.screen.addstr(
                13,
                2,
                "Measured voltage: {0:.4f}mV    ".format(self.eci.measured_voltage * 1000),
            )
            try:
                self.screen.addstr(
                    14, 2, "Update rate: {0:.1f}Hz    ".format(1 / self.eci.looptime)
                )
...
class EmissionControl(threading.Thread):
    ...
    def run(self):
        while self.running:
            ...
            self.wanted_voltage = (
                self.pid.WantedPower(self.emission_current) + self.filament["idle_voltage"]
            )
            self.pid.UpdateSetpoint(self.setpoint)
