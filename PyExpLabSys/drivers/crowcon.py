...
class Vortex(Instrument):
        ...
        for retry in range(0, self.retries + 1):
            try:
                ...
                if retry < self.retries:
                    LOGGER.warning(
                        "Communication error in read_register, retrying %s " "out of %s times",
                        retry + 1,
                        self.retries,
                    )
                    continue
    ...
    def read_string(self, *args, **kwargs):
        ...
        for retry in range(0, self.retries + 1):
            try:
                ...
                if retry < self.retries:
                    LOGGER.warning(
                        "Communication error in read_string, retrying %s " "out of %s times",
                        retry + 1,
                        self.retries,
                    )
                    continue
    ...
    def get_number_installed_digital_outputs(self):
        ...
        LOGGER.debug("get_number_installed_digital_outputs returned: %s", num_digital_outputs)
