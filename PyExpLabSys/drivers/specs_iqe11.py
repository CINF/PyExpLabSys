        ...
        while True:
            ...
            try:
                self.screen.addstr(
                    9,
                    2,
                    "Temperature, electronics: {0:.0f}C          ".format(
                        self.sg.status['temperature']
                    ),
                )
                self.screen.addstr(
                    10,
                    2,
                    "Sputter Current: {0:.4f}mA          ".format(
                        self.sg.status['sputter_current']
                    ),
                )
                self.screen.addstr(
                    11,
                ...
                self.screen.addstr(
                    ...
                    "Filament Current: {0:.2f}A          ".format(
                        self.sg.status['filament_current']
                    ),
                )
                self.screen.addstr(
                    13,
                    2,
                    "Emission current: {0:.4f}mA          ".format(
                        self.sg.status['emission_current']
                    ),
                )
                self.screen.addstr(
                    14,
                    2,
                    "Acceleration Voltage: {0:.2f}V          ".format(
                        self.sg.status['accel_voltage']
                    ),
                )
            except ValueError:
                self.screen.addstr(
                    9, 2, "Temperature, electronics: -                   "
                )
                self.screen.addstr(
                    10, 2, "Sputter Current: -                           "
                )
                self.screen.addstr(
                    11, 2, "Filament bias: -                             "
                )
                self.screen.addstr(
                    12, 2, "Filament Current: -                          "
                )
                self.screen.addstr(
                    13, 2, "Emission current: -                          "
                )
                self.screen.addstr(
                    14, 2, "Acceleration Voltage: -                      "
                )

            # self.screen.addstr(16, 2, "Latest error message: " + self.sg.status['error'])

            self.screen.addstr(
                17, 2, "Runtime: {0:.0f}s       ".format(time.time() - self.time)
            )
