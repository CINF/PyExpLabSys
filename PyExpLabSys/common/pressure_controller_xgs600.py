...
from xgs600 import XGS600Driver as xgs600
...
while True:  # While loop that empties queue
    try:
        element = queue.get()
    except queue.Empty:  # 
        break
...
class xgs600Control(threading.Thread):
    ...
    def __init__(self, port, socket_name, codenames, devices, valve_properties, db_saver = None):
        """
        port: directory to port of USB to RS232 communication
        socket_name: name on net for pushsocket
        codenames: names for retrieving all pressures and states of valves
        devices: userlabel
        valve_properties:   a dictionarie of valve with a list of properties to each valve
                            in the form of {valve_name: [channel, device, setpoint_on, setpoint_off], }
                            each valve must have a channel in the XGS600 and a devices to measure the pressure
                            and two setpoints for the pressure for which the valve is on and off 
...
#                print('channel to log:',channel)
                ...
                if valve_setpoint is not 'OFF' and valve_state == False:
                    ...
                    if self.updated[channel-1] == 0:
                        ...
                        print('Not saved to database, due to earlier being saved', channel, valve_state, valve_setpoint)
                    ...
                    #if valve_setpoint is not 'OFF' and valve_state == True:
#                    self.db_saver.save_point_now('microreactorng_valve_'+valve,1)
#                    print('Nice it is open')
#                elif valve_state == False:
#                    print('It is closed and with purpose')
